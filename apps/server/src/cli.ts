...
function argValue(name: "email" | "password" | "id" | "name"): string | undefined {
...
async function main() {
  ...
  if (command === "seed:user") {
    ...
    if (existing) {
      ...
      db.prepare("INSERT INTO users (id,email,password_hash,created_at) VALUES (?,?,?,?)")
        .run(id("usr"), email, passwordHash, nowIso());
  ...
  if (command === "agents:create" || command === "agents:rotate-token") {
    ...
    if (existing) {
      db.prepare("UPDATE agents SET name = ?, token_hash = ? WHERE id = ?").run(name, tokenHash, agentId);
      console.log(`Updated agent ${agentId}`);
    } else {
      db.prepare("INSERT INTO agents (id,name,token_hash,status,created_at) VALUES (?,?,?,?,?)")
        .run(agentId, name, tokenHash, "offline", nowIso());
